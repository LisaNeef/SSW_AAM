...
lati=lat
b_mean=zeros(length(lat),length(lev),2*dtime+1);
for i=1:length(lat)
for j=1:length(lev)
for k=1:2*dtime+1
b_mean(i,j,k)=mean(squeeze(b(i,j,:,k)))/(std(squeeze(b(i,j,:,k)))/sqrt(length(index)))*cos(lat(i)*pi/180)*cos(lat(i)*pi/180);
end
end
end

 size(b_mean)
  a_mean=zeros(length(lat),length(lev),2*dtime+1);
for i=1:length(lat)
for j=1:length(lev)
for k=1:2*dtime+1
if abs(b_mean(i,j,k))>=2.5*cos(lat(i)*pi/180)*cos(lat(i)*pi/180)
a_mean(i,j,k)=b_mean(i,j,k);
else
a_mean(i,j,k)=NaN;
end
end
...
max(max(max(b_mean)))
    close
    graphik(i)=figure(i);
[co,ho]=contourf(lata,leva,squeeze(a_mean(:,:,i))','LevelList',[-90:.1:30],'Color','none');
  Cld = get(ho, 'Children');
  for j=1:length(Cld)
    if strcmp(get(Cld(j), 'Type'), 'patch')
        Iso = get(Cld(j), 'CData');
          set(Cld(j), 'FaceColor',[0.937500000000000	0	0] );
    end
  end
...
title({strcat(num2str(state),' - composit - day:',num2str(dt(i)),':   Student-t \Delta u [m/s]        ')},'FontSize',15);% \Delta T [K]')},'FontSize',15);
...
print(graphik(i),'-dpng',strcat('lev_lat_anom_u_twerte_composit',num2str(state),'_',num2str(dt(i)),'day'))
