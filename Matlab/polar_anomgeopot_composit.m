...
function polar_anomgeopot_composit(index,cdate,para1,lat,lon,dtime, state)
dt=-dtime:dtime;
...
b_mean=zeros(length(lon),length(lat),2*dtime+1);
for i=1:length(lon)
for j=1:length(lat)
for k=1:2*dtime+1
b_mean(i,j,k)=mean(squeeze(a(i,j,:,k)))/(std(squeeze(a(i,j,:,k)))/sqrt(length(index)));
end
end
end

a_mean=mean(a,3);
t_wert=tinv(0.9,length(index)-2);
t_wert

a_mean1=zeros(length(lon),length(lat),2*dtime+1);
for i=1:length(lon)
for j=1:length(lat)
for k=1:2*dtime+1
if abs(b_mean(i,j,k))>=abs(t_wert)
a_mean1(i,j,k)=b_mean(i,j,k);
else
a_mean1(i,j,k)=NaN;
end
end
...
for i=1:length(dt_plot)
   ...
   load coast;
        ...
        %    hold on
            ...
            %gfz
            [ci,hi]=contourfm(lata,lona,double(squeeze(a_mean1(:,:,dt_plot(i)+dtime+1)))/10,-600:2:0,'k--'); 
            hold on
            [cis,his]=contourfm(lata,lona,double(squeeze(a_mean1(:,:,dt_plot(i)+dtime+1)))/10,0:2:600,'LineColor','k'); 
...
title({strcat(num2str(state),' - composit - day:',num2str(dt_plot(i)),':  \Delta gph [gdm]       ')},'FontSize',15);% \Delta T [K]')},'FontSize',15);


%Speichern
set(graphik(i),'PaperPositionMode','auto');
print(graphik(i),'-dpng',strcat('polar500_anomgeopot_composit',num2str(state),'_',num2str(dt_plot(i)),'day'))
